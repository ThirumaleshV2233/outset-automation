...
test.describe('The Outset – Homepage Navbar', () => {
    ...
    test('should open search overlay, accept a query, and navigate to results page', async ({ page }) => {
        await navbar.searchFor('sunscreen');

        await expect(page).toHaveURL(/search.*q=sunscreen/);
        await expect(page.locator('body')).not.toContainText('404');
    });

    test('should show a cart count of 0 for guest users', async () => {
        await navbar.verifyCartCount('0');
    });
    ...
    test('should display Instagram and TikTok social links in the mobile menu', async ({ page }) => {
        ...
        await expect(page.locator('#navMenuWrapper a[href="https://www.instagram.com/theoutset/"]').first()).toBeVisible();
        await expect(page.locator('#navMenuWrapper a[href="https://www.tiktok.com/@theoutset"]').first()).toBeVisible();
    });
